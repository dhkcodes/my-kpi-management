...
assert.match(insightsPage, /analysis\.movementBridge\.some\(\(point\) => point\.compositionStatus === "UNCLASSIFIED"\)[\s\S]*Confirmed New, Expansion, and Reduction amounts are shown; Total Forecast remains unchanged/, "incomplete composition data is labelled while confirmed components remain visible");
assert.match(insightsPage, /Unclassified included · confirmed components only/, "partially classified quarters explicitly label confirmed-only composition values");
