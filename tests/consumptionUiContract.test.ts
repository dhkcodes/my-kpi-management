...
assert.match(recordsPage, /oj-ux-ico-download[\s\S]*Export CSV[\s\S]*oj-ux-ico-upload[\s\S]*Actual Import[\s\S]*oj-ux-ico-upload[\s\S]*Forecast Import/, "Export, Actual Import, and Forecast Import are adjacent Redwood actions with clear icons");
...
assert.match(recordsPage, /saveConsumptionForecasts\(apiEtag, controlUpdates\)/, "Forecast API integration sends only Account-level control updates");
assert.match(recordsPage, /const editable = selectedPillar !== "ALL" && editablePeriodIds\.has\(month\)[\s\S]*const canEditControl = editable && Boolean\(resolution\?\.editable\)/, "DP and OCI-Other Account Forecasts are independently editable while ALL remains read-only");
