...
assert.match(insightsPage, /analysis\.movementBridge\.some\(\(point\) => point\.compositionStatus === "UNCLASSIFIED"\)[\s\S]*classified components are hidden to avoid presenting an unreconciled partial composition/, "incomplete composition data is explained instead of silently appearing absent");
