...
assert.match(recordsPage, /fetchConsumptionWorkspace\(\{ fromQuarter, toQuarter \}\)/, "Data range reloads the existing workspace API");
...
assert.match(recordsPage, /offset:\s*append \? recordsNextOffset : 0[\s\S]*sort:\s*recordSort\.toUpperCase\(\)[\s\S]*direction:\s*recordDirection\.toUpperCase\(\)/, "every records page sends the authoritative range, sort and offset query");
...
assert.match(recordsPage, /disabled=\{hasDraftChanges \|\| recordsLoading\}/, "search and sort controls are locked while drafts or a request are active");
...
assert.match(recordsPage, /data-readonly=\{expandable \? "account" : undefined\}/, "aggregate account rows remain explicitly read-only");
...
assert.match(styles, /\.consumption-record-controls[\s\S]*\.consumption-load-more/, "Usage Records controls and Load More have scoped styles");
assert.match(styles, /\.consumption-table-scroll\s*\{[^}]*overflow-x:\s*auto/, "the wide Data grid owns horizontal scrolling");
...
assert.match(styles, /\.consumption-table thead tr:nth-child\(2\) th\s*\{[^}]*position:\s*sticky[^}]*top:\s*3\.25rem/, "second header row remains sticky");
