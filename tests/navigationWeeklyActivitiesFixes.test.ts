...
assert.match(cssSource, /\.kpi-menu-group\s*\{[^}]*border:\s*0;[^}]*border-bottom:\s*1px solid #d8d3cf/s, "menu groups use a single understated hairline instead of rectangular boxes");
...
assert.match(cssSource, /\.kpi-menu-link\s*\{[^}]*background:\s*#fff;[^}]*border:\s*1px solid #bdb6b1/s, "Redwood Hairline default badges use the approved neutral surface and border");
assert.match(cssSource, /\.kpi-menu-link:hover\s*\{[\s\S]*background:\s*#fbe9e7[\s\S]*border-color:\s*#b43b2c[\s\S]*box-shadow:\s*inset 3px 0 #c74634[\s\S]*color:\s*#6e251b/, "hover uses the approved Redwood Hairline accent treatment");
